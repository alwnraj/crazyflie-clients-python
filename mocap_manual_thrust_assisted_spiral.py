#!/usr/bin/env python3
"""
Manual-thrust Crazyflie flight with mocap-assisted roll, pitch, yaw, and spiral.

This is intentionally a script you tune by editing the constants below. The
pilot owns takeoff and landing thrust. During the spiral, the script can
...
- optional spiral-only altitude hold correction on top of pilot thrust
...
6. Press F to lock the current X/Y as the spiral center and start the spiral. Press F again to
   return to the spiral center and land.
7. During 3ft hold or spiral, Up/Down nudge the height target.
...
SPIRAL_TRACKING_RESERVE_M = 0.18
...
SPIRAL_TARGET_ERROR_LIMIT_M = 1.35
SPIRAL_STARTUP_TARGET_ERROR_LIMIT_M = 1.75
TARGET_ERROR_GRACE_S = 0.30
SPIRAL_TARGET_ERROR_GRACE_S = 0.70
MAX_TAKEOFF_TARGET_ERROR_M = 1.20
SPIRAL_DRIFT_SAFETY_MARGIN_M = 0.20
...
MOCAP_STALE_RESUME_SPIRAL_S = 3.50
...
# Pre-spiral height helper. Press T after takeoff to climb/hold about 3 ft
# before pressing F. It uses mocap height and is disabled during stale mocap.
PRESPIRAL_HEIGHT_HOLD_ENABLED = True
PRESPIRAL_HEIGHT_TARGET_M = 0.9144
PRESPIRAL_HEIGHT_MAX_TARGET_M = 1.20
PRESPIRAL_HEIGHT_MIN_TARGET_M = 0.10
PRESPIRAL_HEIGHT_READY_ERROR_M = 0.12
PRESPIRAL_HEIGHT_READY_VERTICAL_SPEED_M_S = 0.08
PRESPIRAL_BASE_THRUST_RAW = 34000
PRESPIRAL_ALTITUDE_KP_RAW_PER_M = 5200.0
PRESPIRAL_ALTITUDE_KI_RAW_PER_M_S = 650.0
PRESPIRAL_ALTITUDE_KD_RAW_PER_M_S = 3800.0
PRESPIRAL_ALTITUDE_CORRECTION_LIMIT_RAW = 1800.0
PRESPIRAL_ALTITUDE_CORRECTION_SLEW_RAW_PER_S = 2600.0
...
# Outward-and-inward spiral target. F locks the current X/Y as the spiral
# center. The target eases from the center, expands to its maximum radius,
# contracts, and then repeats.
SPIRAL_ENTRY_S = 5.0
SPIRAL_MIN_RADIUS_M = 0.30
SPIRAL_MAX_RADIUS_M = 2.60
SPIRAL_ROTATION_PERIOD_S = 60.0
SPIRAL_OUT_AND_BACK_S = 120.0
SPIRAL_MIN_MAX_RADIUS_M = 0.60
SPIRAL_MAX_START_HORIZONTAL_SPEED_M_S = 0.20
SPIRAL_MAX_START_VERTICAL_SPEED_M_S = 0.08

# Spiral altitude hold. This only runs while spiral mode is active and
# mocap is fresh. Up/Down change the target height during spiral; outside
# spiral they still change raw thrust.
SPIRAL_ALTITUDE_HOLD_ENABLED = True
SPIRAL_ALTITUDE_STEP_M = 0.03
SPIRAL_ALTITUDE_BIG_STEP_M = 0.08
SPIRAL_ALTITUDE_MIN_TARGET_M = 0.04
SPIRAL_ALTITUDE_MAX_TARGET_M = 1.20
SPIRAL_ALTITUDE_KP_RAW_PER_M = 6500.0
SPIRAL_ALTITUDE_KI_RAW_PER_M_S = 900.0
SPIRAL_ALTITUDE_KD_RAW_PER_M_S = 3500.0
SPIRAL_ALTITUDE_INTEGRAL_MAX_ERROR_S = 0.60
SPIRAL_ALTITUDE_CORRECTION_LIMIT_RAW = 1800.0
SPIRAL_ALTITUDE_CORRECTION_SLEW_RAW_PER_S = 3000.0
...
class CsvLogger:
    FIELDNAMES = """
        ...
        spiral_target_error_limit_m spiral_active return_land_active
        return_home_error_m spiral_elapsed_s spiral_requested_max_radius_m
        spiral_min_radius_m spiral_max_radius_m
        spiral_width_m spiral_height_m spiral_path_min_x
        spiral_path_max_x spiral_path_min_y spiral_path_max_y
        spiral_wall_margin_m spiral_shrunk_to_cage
        spiral_altitude_hold_active spiral_target_height_m
        spiral_height_error_m spiral_altitude_integral_error_s
        spiral_altitude_correction_raw height_assist_mode height_assist_active
        prespiral_height_hold_active prespiral_target_height_m
        prespiral_height_ready mocap_x mocap_y mocap_z mocap_qx mocap_qy
    ...
    def __init__(self):
        ...
        self.output_path = Path(OUTPUT_DIR) / f"mocap-assisted-spiral-{timestamp}.csv"
...
def make_spiral_profile(center_x, center_y):
    planning_bounds = cage_bounds(CAGE_WALL_MARGIN_M + SPIRAL_TRACKING_RESERVE_M)
    ...
    max_radius_m = min(SPIRAL_MAX_RADIUS_M, max_radius)
    if max_radius_m < SPIRAL_MIN_MAX_RADIUS_M:
        return None

    min_radius_m = min(SPIRAL_MIN_RADIUS_M, 0.5 * max_radius_m)
    extents = {
        "min_x": center_x - max_radius_m,
        "max_x": center_x + max_radius_m,
        "min_y": center_y - max_radius_m,
        "max_y": center_y + max_radius_m,
    }
    margin = bounds_margin(extents, hard_bounds)
    shrunk = max_radius_m < SPIRAL_MAX_RADIUS_M

    if shrunk:
        message = (
            f"Spiral maximum radius shrunk to {max_radius_m:.2f}m; "
            f"wall margin {margin:.2f}m."
        )
    else:
        message = (
            f"Spiral maximum radius {max_radius_m:.2f}m; "
            f"wall margin {margin:.2f}m."
        )
...
def spiral_target(center_x, center_y, elapsed_s, profile):
    if elapsed_s < SPIRAL_ENTRY_S:
        progress = elapsed_s / SPIRAL_ENTRY_S
        eased = 0.5 - 0.5 * math.cos(math.pi * progress)
        radius = profile.min_radius_m * eased
        theta = 0.0
        return center_x + radius, center_y

    spiral_t = elapsed_s - SPIRAL_ENTRY_S
    cycle_progress = (spiral_t % SPIRAL_OUT_AND_BACK_S) / SPIRAL_OUT_AND_BACK_S
    if cycle_progress < 0.5:
        radius_progress = cycle_progress / 0.5
    else:
        radius_progress = 1.0 - ((cycle_progress - 0.5) / 0.5)

    eased_radius_progress = 0.5 - 0.5 * math.cos(math.pi * radius_progress)
    radius = (
        profile.min_radius_m
        + (profile.max_radius_m - profile.min_radius_m) * eased_radius_progress
    )
    theta = 2.0 * math.pi * (spiral_t / SPIRAL_ROTATION_PERIOD_S)
    return (
...
def draw(stdscr, state):
    stdscr.erase()
    spiral_status = (
        "ON" if state["spiral_active"] else
        "RETURN" if state["return_land_active"] else
        "READY" if state["spiral_ready"] else
        "off"
    )
    lines = [
        (0, "Manual Thrust + Mocap Assisted Spiral"),
        (2, "Controls: R ready | T 3ft hold | Up/Down thrust or Z target | PgDn descent"),
        (3, "Trim: W/S pitch +/- | A/D roll -/+ | J/L yaw target -/+ | C clear"),
        (4, "F start spiral / return+land | H lock X/Y | Space cut | Q/Esc cut+quit"),
        ...
        (16, f"Spiral: {spiral_status} | elapsed={state['spiral_elapsed']:.1f}s "
             f"| target offset=({state['spiral_target_dx']:+.3f}, {state['spiral_target_dy']:+.3f})"),
        (17, f"Path: {state['spiral_width']:.2f}m x {state['spiral_height']:.2f}m "
             f"| wall margin={state['spiral_wall_margin']:.2f}m | shrunk={state['spiral_shrunk']}"),
        ...
        (19, f"3ft ready: {'YES' if state['prespiral_height_ready'] else 'no'} "
             f"| hold={'ON' if state['prespiral_height_hold_active'] else 'off'}"),
...
def run_control_loop(stdscr, cf, mocap_state, mocap_reader, telemetry, start_position, start_quat, logger):
    ...
    spiral_active = False
    spiral_started_at = None
    spiral_profile = None
    spiral_target_height = None
    prespiral_height_hold_active = False
    prespiral_target_height = PRESPIRAL_HEIGHT_TARGET_M
    ...
    stale_saved_spiral_active = False
    stale_saved_spiral_started_at = None
    stale_saved_spiral_profile = None
    stale_saved_spiral_target_height = None
    ...
    def start_safety_descent(reason):
        ...
        nonlocal spiral_active, spiral_started_at, spiral_profile
        nonlocal spiral_target_height, altitude_hold_correction, altitude_integral
        nonlocal prespiral_height_hold_active
    ...
    def clamp_spiral_height_target(value):
        if ENFORCE_HEIGHT_LIMITS:
            upper = min(SPIRAL_ALTITUDE_MAX_TARGET_M, MAX_HEIGHT_ABOVE_START_M - 0.10)
        else:
            upper = SPIRAL_ALTITUDE_MAX_TARGET_M
        upper = max(SPIRAL_ALTITUDE_MIN_TARGET_M, upper)
        return clamp(value, SPIRAL_ALTITUDE_MIN_TARGET_M, upper)

    def clamp_prespiral_height_target(value):
        return clamp(
            value,
            PRESPIRAL_HEIGHT_MIN_TARGET_M,
            PRESPIRAL_HEIGHT_MAX_TARGET_M,
        )
    ...
    while True:
        ...
        if mocap_stale:
            if stale_started_at is None:
                ...
                stale_saved_spiral_active = spiral_active
                stale_saved_spiral_started_at = spiral_started_at
                stale_saved_spiral_profile = spiral_profile
                stale_saved_spiral_target_height = spiral_target_height
                prespiral_height_hold_active = False
                spiral_active = False
                spiral_started_at = None
                spiral_profile = None
            ...
            integral_y = 0.0
            if (
                stale_saved_spiral_active
                and stale_for <= MOCAP_STALE_RESUME_SPIRAL_S
                and stale_saved_spiral_profile is not None
            ):
                spiral_active = True
                spiral_started_at = stale_saved_spiral_started_at
                spiral_profile = stale_saved_spiral_profile
                spiral_target_height = stale_saved_spiral_target_height
                return_land_active = False
                return_land_descent_started = False
                message = f"Mocap reacquired after {stale_for:.1f}s; resuming spiral."
        ...
        if key == ord(" "):
            ...
            elif spiral_active and SPIRAL_ALTITUDE_HOLD_ENABLED:
                if spiral_target_height is None:
                    spiral_target_height = clamp_spiral_height_target(current_height)
                spiral_target_height = clamp_spiral_height_target(
                    spiral_target_height + SPIRAL_ALTITUDE_STEP_M
                )
                message = f"Spiral height target {spiral_target_height:.2f}m."
            elif prespiral_height_hold_active and PRESPIRAL_HEIGHT_HOLD_ENABLED:
                prespiral_target_height = clamp_prespiral_height_target(
                    prespiral_target_height + SPIRAL_ALTITUDE_STEP_M
                )
                message = f"3ft height target {prespiral_target_height:.2f}m."
            else:
            ...
            if spiral_active and SPIRAL_ALTITUDE_HOLD_ENABLED:
                if spiral_target_height is None:
                    spiral_target_height = clamp_spiral_height_target(current_height)
                spiral_target_height = clamp_spiral_height_target(
                    spiral_target_height - SPIRAL_ALTITUDE_STEP_M
            ...
            if safety_descent_active:
                ...
                if spiral_target_height is None:
                    spiral_target_height = clamp_spiral_height_target(current_height)
                spiral_target_height = clamp_spiral_height_target(
                    spiral_target_height + SPIRAL_ALTITUDE_BIG_STEP_M
                )
                message = f"Spiral height target {spiral_target_height:.2f}m."
            elif prespiral_height_hold_active and PRESPIRAL_HEIGHT_HOLD_ENABLED:
                prespiral_target_height = clamp_prespiral_height_target(
                    prespiral_target_height + SPIRAL_ALTITUDE_BIG_STEP_M
                )
                message = f"3ft height target {prespiral_target_height:.2f}m."
            else:
            ...
            if safety_descent_active:
            ...
            elif spiral_active or return_land_active:
                message = "3ft height hold is only for before spiral."
            elif prespiral_height_hold_active:
                prespiral_height_hold_active = False
                altitude_hold_correction = 0.0
                altitude_integral = 0.0
                ...
                prespiral_target_height = clamp_prespiral_height_target(
                    PRESPIRAL_HEIGHT_TARGET_M
                )
                prespiral_height_hold_active = True
                target_thrust = int(
                    max(target_thrust, PRESPIRAL_BASE_THRUST_RAW)
                )
                descent_active = False
                altitude_hold_correction = 0.0
                altitude_integral = 0.0
                message = (
                    f"3ft height hold on: target {prespiral_target_height:.2f}m. "
            ...
            if mocap_stale:
                message = "Cannot start spiral while mocap is stale."
            elif spiral_active:
                spiral_active = False
                spiral_started_at = None
                spiral_profile = None
                ...
                integral_x = 0.0
                integral_y = 0.0
                message = "Returning to spiral center; landing when close."
                ...
                requested_profile = make_spiral_profile(position[0], position[1])
                if abs(velocity_z) > SPIRAL_MAX_START_VERTICAL_SPEED_M_S:
                    message = (
                        f"Spiral rejected: vertical speed {velocity_z:+.2f}m/s "
                        f"exceeds {SPIRAL_MAX_START_VERTICAL_SPEED_M_S:.2f}m/s."
                    )
                elif horizontal_start_speed > SPIRAL_MAX_START_HORIZONTAL_SPEED_M_S:
                    message = (
                        f"Spiral rejected: XY speed {horizontal_start_speed:.2f}m/s "
                        f"exceeds {SPIRAL_MAX_START_HORIZONTAL_SPEED_M_S:.2f}m/s."
                    )
                elif requested_profile is None:
                    message = "Spiral rejected: start point is too close to the cage wall."
                else:
                    hold_x, hold_y = position[0], position[1]
                    target_x, target_y = hold_x, hold_y
                    hold_target_frozen = True
                    ...
                    spiral_profile = requested_profile
                    spiral_target_height = clamp_spiral_height_target(current_height)
                    prespiral_height_hold_active = False
                    altitude_hold_correction = 0.0
                    altitude_integral = 0.0
                    ...
                    integral_x = 0.0
                    integral_y = 0.0
                    if SPIRAL_ALTITUDE_HOLD_ENABLED:
                        message = (
                            f"{spiral_profile.message} Z hold target "
                            f"{spiral_target_height:.2f}m."
                        )
                    else:
                        message = f"{spiral_profile.message} Keep altitude with thrust."
        ...
        altitude_kp = SPIRAL_ALTITUDE_KP_RAW_PER_M
        altitude_ki = SPIRAL_ALTITUDE_KI_RAW_PER_M_S
        altitude_kd = SPIRAL_ALTITUDE_KD_RAW_PER_M_S
        altitude_correction_limit = SPIRAL_ALTITUDE_CORRECTION_LIMIT_RAW
        altitude_correction_slew = SPIRAL_ALTITUDE_CORRECTION_SLEW_RAW_PER_S
        if (
            SPIRAL_ALTITUDE_HOLD_ENABLED
            and (spiral_active or return_land_active)
            and not mocap_stale
            and not safety_descent_active
            and not descent_active
            and spiral_target_height is not None
        ):
            height_assist_mode = "return-home" if return_land_active else "spiral"
            spiral_target_height = clamp_spiral_height_target(spiral_target_height)
            height_assist_target = spiral_target_height
        elif (
            PRESPIRAL_HEIGHT_HOLD_ENABLED
            and prespiral_height_hold_active
            and not mocap_stale
            and not safety_descent_active
            and not descent_active
            and not return_land_active
        ):
            height_assist_mode = "prespiral"
            prespiral_target_height = clamp_prespiral_height_target(
                prespiral_target_height
            )
            height_assist_target = prespiral_target_height
            target_thrust = max(target_thrust, PRESPIRAL_BASE_THRUST_RAW)
            altitude_kp = PRESPIRAL_ALTITUDE_KP_RAW_PER_M
            altitude_ki = PRESPIRAL_ALTITUDE_KI_RAW_PER_M_S
            altitude_kd = PRESPIRAL_ALTITUDE_KD_RAW_PER_M_S
            altitude_correction_limit = PRESPIRAL_ALTITUDE_CORRECTION_LIMIT_RAW
            altitude_correction_slew = PRESPIRAL_ALTITUDE_CORRECTION_SLEW_RAW_PER_S
        ...
        if altitude_hold_active:
            ...
            altitude_integral = clamp(
                ...
                -SPIRAL_ALTITUDE_INTEGRAL_MAX_ERROR_S,
                SPIRAL_ALTITUDE_INTEGRAL_MAX_ERROR_S,
            )
        ...
        if not mocap_stale:
            ...
            if spiral_active and spiral_profile is not None:
                spiral_center_offset = math.hypot(
                    hold_x - start_x,
                    hold_y - start_y,
                )
                spiral_drift_limit = (
                    spiral_center_offset
                    + spiral_profile.max_radius_m
                    + SPIRAL_DRIFT_SAFETY_MARGIN_M
                )
                xy_drift_limit = max(xy_drift_limit, spiral_drift_limit)
        ...
        if safety_descent_active:
            ...
            spiral_active
            and spiral_started_at is not None
            and spiral_profile is not None
        ):
            target_x, target_y = spiral_target(
                hold_x,
                hold_y,
                now - spiral_started_at,
                spiral_profile,
            )
            phase = "spiral"
        else:
        ...
        if spiral_active and spiral_started_at is not None:
            spiral_elapsed = now - spiral_started_at
        else:
            spiral_elapsed = 0.0
        ...
        prespiral_height_error = prespiral_target_height - height
        prespiral_height_ready = (
            not mocap_stale
            and abs(prespiral_height_error) <= PRESPIRAL_HEIGHT_READY_ERROR_M
            and abs(velocity_z) <= PRESPIRAL_HEIGHT_READY_VERTICAL_SPEED_M_S
        )
        if (
            prespiral_height_hold_active
            and not safety_descent_active
            and not descent_active
            ...
            if prespiral_height_ready:
                message = "3ft height ready; press F to start spiral."
            else:
                message = (
                    f"3ft height hold: {prespiral_height_error:+.2f}m "
                    "from target."
                )
        if (
            return_land_active
            ...
            if (
                ...
                target_thrust = min(target_thrust, thrust)
                message = "At spiral center; slow landing ramp active."
            else:
                message = (
                    f"Returning to spiral center: {return_home_error:.2f}m away. "
                    "Landing when close."
                )
        if spiral_active:
            prospective_spiral_profile = spiral_profile
        else:
            prospective_spiral_profile = make_spiral_profile(position[0], position[1])
        spiral_ready = (
            not mocap_stale
            and not safety_descent_active
            and not spiral_active
            and not return_land_active
            and prospective_spiral_profile is not None
            and speed <= SPIRAL_MAX_START_HORIZONTAL_SPEED_M_S
            and abs(velocity_z) <= SPIRAL_MAX_START_VERTICAL_SPEED_M_S
        )
        displayed_spiral_profile = (
            spiral_profile
            if spiral_profile is not None
            else prospective_spiral_profile
        )
        if displayed_spiral_profile is None:
            spiral_min_radius_m = 0.0
            spiral_max_radius_m = 0.0
            spiral_path_min_x = 0.0
            spiral_path_max_x = 0.0
            spiral_path_min_y = 0.0
            spiral_path_max_y = 0.0
            spiral_wall_margin = 0.0
            spiral_shrunk = 0
        else:
            spiral_min_radius_m = displayed_spiral_profile.min_radius_m
            spiral_max_radius_m = displayed_spiral_profile.max_radius_m
            spiral_path_min_x = displayed_spiral_profile.min_x
            spiral_path_max_x = displayed_spiral_profile.max_x
            spiral_path_min_y = displayed_spiral_profile.min_y
            spiral_path_max_y = displayed_spiral_profile.max_y
            spiral_wall_margin = displayed_spiral_profile.margin_m
            spiral_shrunk = int(displayed_spiral_profile.shrunk)
        if safety_descent_active:
            target_error_limit = RETURN_HOME_TARGET_ERROR_LIMIT_M
        elif return_land_active:
            target_error_limit = RETURN_HOME_TARGET_ERROR_LIMIT_M
        elif spiral_active and spiral_profile is not None:
            if spiral_elapsed < SPIRAL_ENTRY_S:
                target_error_limit = max(
                    SPIRAL_TARGET_ERROR_LIMIT_M,
                    SPIRAL_STARTUP_TARGET_ERROR_LIMIT_M,
                )
            else:
                target_error_limit = SPIRAL_TARGET_ERROR_LIMIT_M
        else:
            target_error_limit = (
                MAX_TARGET_ERROR_M
                if height >= FULL_AUTHORITY_HEIGHT_M
        ...
        if mocap_stale or assist_blend <= 0.0:
            ...
            elif spiral_active and height >= FULL_AUTHORITY_HEIGHT_M:
                base_angle_limit = max(base_angle_limit, SPIRAL_MAX_ANGLE_DEG)
        ...
        if should_log:
            logger.write({
                ...
                "spiral_target_error_limit_m": target_error_limit,
                "spiral_active": int(spiral_active),
                "return_land_active": int(return_land_active),
                "return_home_error_m": return_home_error,
                "spiral_elapsed_s": spiral_elapsed,
                "spiral_requested_max_radius_m": SPIRAL_MAX_RADIUS_M,
                "spiral_min_radius_m": spiral_min_radius_m,
                "spiral_max_radius_m": spiral_max_radius_m,
                "spiral_width_m": 2.0 * spiral_max_radius_m,
                "spiral_height_m": 2.0 * spiral_max_radius_m,
                "spiral_path_min_x": spiral_path_min_x,
                "spiral_path_max_x": spiral_path_max_x,
                "spiral_path_min_y": spiral_path_min_y,
                "spiral_path_max_y": spiral_path_max_y,
                "spiral_wall_margin_m": spiral_wall_margin,
                "spiral_shrunk_to_cage": spiral_shrunk,
                "spiral_altitude_hold_active": int(altitude_hold_active),
                "spiral_target_height_m": (
                    spiral_target_height
                    if spiral_target_height is not None
                    else ""
                ),
                "spiral_height_error_m": altitude_height_error,
                "spiral_altitude_integral_error_s": altitude_integral,
                "spiral_altitude_correction_raw": altitude_hold_correction,
                "height_assist_mode": height_assist_mode,
                "height_assist_active": int(altitude_hold_active),
                "prespiral_height_hold_active": int(prespiral_height_hold_active),
                "prespiral_target_height_m": prespiral_target_height,
                "prespiral_height_ready": int(prespiral_height_ready),
                "mocap_x": position[0],
                "mocap_y": position[1],
                "mocap_z": position[2],
                "mocap_qx": quat.x,
        ...
        if now - last_draw_at >= 0.10:
            draw(stdscr, {
                ...
                "spiral_active": spiral_active,
                "return_land_active": return_land_active,
                "return_home_error": return_home_error,
                "spiral_ready": spiral_ready,
                "spiral_elapsed": spiral_elapsed,
                "spiral_target_dx": target_x - hold_x,
                "spiral_target_dy": target_y - hold_y,
                "spiral_width": 2.0 * spiral_max_radius_m,
                "spiral_height": 2.0 * spiral_max_radius_m,
                "spiral_wall_margin": spiral_wall_margin,
                "spiral_shrunk": spiral_shrunk,
                ...
                "height_assist_mode": height_assist_mode,
                "prespiral_height_hold_active": prespiral_height_hold_active,
                "prespiral_height_ready": prespiral_height_ready,
                "drift_x": drift_x,
                "drift_y": drift_y,
                "drift": drift,
                "vx": velocity_x,
...
def main():
    ...
    startup_lines = [
        "=" * 72,
        "MANUAL THRUST + MOCAP ASSISTED SPIRAL",
        "=" * 72,
        f"URI: {URI}",
        ...
        f"PgUp +{BIG_THRUST_STEP}; during 3ft/spiral hold these nudge the Z target",
        f"3ft helper: T toggles target {PRESPIRAL_HEIGHT_TARGET_M:.2f}m "
        f"({PRESPIRAL_HEIGHT_TARGET_M * 3.28084:.1f}ft), "
        f"ready within +/-{PRESPIRAL_HEIGHT_READY_ERROR_M:.2f}m",
        ...
        f"Spiral request: radius {SPIRAL_MIN_RADIUS_M:.2f}m to "
        f"{SPIRAL_MAX_RADIUS_M:.2f}m, rotation {SPIRAL_ROTATION_PERIOD_S:.1f}s, "
        f"out-and-back {SPIRAL_OUT_AND_BACK_S:.1f}s, entry {SPIRAL_ENTRY_S:.1f}s, "
        f"spiral angle cap {SPIRAL_MAX_ANGLE_DEG:.1f}deg; "
        "maximum radius auto-shrinks if the center is too close to a wall",
        f"Spiral Z hold: enabled={SPIRAL_ALTITUDE_HOLD_ENABLED}, "
        f"step={SPIRAL_ALTITUDE_STEP_M:.2f}m, "
        f"correction <= +/-{SPIRAL_ALTITUDE_CORRECTION_LIMIT_RAW:.0f} raw",
