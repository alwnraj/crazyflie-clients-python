...
#Determines if we are using Ricky's UDP Stream or one of the Mocap systems to receive pose data
using_mocap = True
...
def send_extpose(cf_local, sock):
    ...
    try:
        while True:
            ...
            if len(response) == 29:
                ...
                cf_local.send_extpose([x, z, y], [qx, qz, qy, qw])#Motive uses y as vertical axis, so we flip z and y
...
def main():
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        if using_mocap:
            ...
            sock.bind(("127.0.0.1", 10444)) #Bind socket to our specified port
            cf_local = Localization(crazyflie = scf.cf)
            thread = Thread(target=send_extpose, args=(cf_local, sock), daemon=True)
            thread.start()
